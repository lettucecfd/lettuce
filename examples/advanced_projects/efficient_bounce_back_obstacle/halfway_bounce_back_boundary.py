...
class HalfwayBounceBackBoundary(Boundary):
    ...
    """
        HALFWAY BOUNCE BACK BOUNDARY CONDITION (HWBB):
        - inverts populations on fluid nodex, neighboring boundary
        - improved temporal accuracy in comparison to FWBB
    ...
    def __init__(self, context, flow, solid_boundary_data: SolidBoundaryData,
        ...
        # global_solid_mask to filter out all "fake" fluid neighbors,
        #   which are outside this HWBB but not in the fluid region
        ...
        # COMPATIBILITY to unified Solid Boundary Data object!
        # -> combine f_index_lt and f_index_gt (needed for IBB1) to self.f_index

        # if solid_boundary_data contains batch_indices, use them
        if ((hasattr(solid_boundary_data, "f_index_gt")
            or hasattr(solid_boundary_data, "f_index_lt"))
                and len(solid_boundary_data.f_index_lt.shape)
                == len(solid_boundary_data.f_index_gt.shape)):
            print("HWBB: SBD has got attributs f_index_gt and f_index_lt!")
            self.f_index = np.concatenate((solid_boundary_data.f_index_lt,
                                           solid_boundary_data.f_index_gt), axis=0)
        elif (hasattr(solid_boundary_data, "f_index_gt")
              and solid_boundary_data.f_index_lt.shape[0] == 0):
            print("HWBB: SBD has got attributf_index_gt (NO f_index_lt)!")
            self.f_index = solid_boundary_data.f_index_gt
        elif (hasattr(solid_boundary_data, "f_index_lt")
            ...
            if self.flow.stencil.d == 2:
                ...
                for p in range(0, len(a)):  # for all TRUE-nodes in boundary.mask
                    for i in range(0, self.flow.stencil.q):
                        ...
                        if (a[p] == 0 and self.flow.stencil.e[i][ 0] == -1
                                and periodicity[0]):  # searching border on left [x]
                            border[0] = -1
                        elif (a[p] == nx - 1 and self.flow.stencil.e[i][ 0] == 1
                              and periodicity[0]):  # searching border on right [x]
                            border[0] = 1

                        if (b[p] == 0 and self.flow.stencil.e[i][ 1] == -1
                                and periodicity[1]):  # searching border on left [y]
                            border[1] = -1
                        elif (b[p] == ny - 1 and self.flow.stencil.e[i][ 1] == 1
                              and periodicity[1]):  # searching border on right [y]
                            border[1] = 1
                ...
                for p in range(0, len(a)):
                    for i in range(0, self.flow.stencil.q):
                        ...
                        if (b[p] == 0 and self.flow.stencil.e[i][ 1] == -1
                            ...
                            border[1] = 1
                        # z - direction
                        if (c[p] == 0 and self.flow.stencil.e[i][ 2] == -1
                                and periodicity[2]):  # searching border on left
                            border[2] = -1
                        elif (c[p] == nz - 1 and self.flow.stencil.e[i][ 2] == 1
        ...
        self.f_index = torch.tensor(np.array(self.f_index),
    ...
    def __call__(self, flow):
        ...
        #   f = torch.where(self.f_mask[self.flow.stencil.opposite],
        #                   f_collided[self.flow.stencil.opposite], f)
    ...
    def make_no_collision_mask(self, f_shape: List[int], context: 'Context'
        ...
        # INFO: for the halfway bounce back boundary,
        #       a no_collision_mask ist not necessary, because the no_stream_mask
        #       prevents interaction between nodes inside and outside the boundary region.
        # INFO: pay attention to the initialization of observable/moment-fields
        #       (u, rho,...) on the boundary nodes, in the initial solution of your flow,
    ...
    def calc_force_on_boundary(self):
        """calculate the fluid force on boundary by momentum exchange"""
        # calculate force on boundary by momentum exchange method (MEA, MEM)
        # according to Kruger et al., 2017, pp.215-217:
        # momentum (f_i*c_i - f_i_opposite*c_i_opposite = 2*f_i*c_i for a resting boundary)
        # is summed for all populations pointing at the surface of the boundary
    ...
    def store_f_collided(self, f_collided):
        ...
        if self.flow.torch_stencil.d == 2:
            self.f_collided[:, 0] = torch.clone(
                f_collided[self.f_index[:, 0],  # q
                            self.f_index[:, 1],  # x
                            self.f_index[:, 2]])  # y
            self.f_collided[:, 1] = torch.clone(
                f_collided[self.opposite_tensor[self.f_index[:, 0]],  # q
                                                self.f_index[:, 1],  # x
                                                self.f_index[:, 2]])  # y
        if self.flow.torch_stencil.d == 3:
            self.f_collided[:, 0] = torch.clone(

...
class ExtFlow(Flow, ABC):
    """
    Most __init__ methods of Flow look the same while having a lot of
    parameters. This class implements a common parameter set and allows
    subclasses to only implement the creation of the unit conversion and
    resolution.
    """
    ...
    def __init__(self, context: 'Context', resolution: Union[int, List[int]],
        ...
        Flow.__init__(self, context, resolution, self.make_units(
            reynolds_number, mach_number, resolution), stencil, equilibrium)

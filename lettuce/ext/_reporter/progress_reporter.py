...
class ProgressReporter(Reporter):
    '''
        Progress reporter that logs: current wall time, elapsed wall time, elapsed steps and estimates wall time
        remaining.
        Option to write a checkpoint file, when t_max is reached. (Sim. can be restarted from checkpoint)

        (!) This reporter does not export other reporters observable values etc.,
        so make sure you save them in other ways, if sim. is stopped by host system (e.g. HPC cluster)!
    ...
    def __init__(self, interval=1000, t_max=0, i_target=0, outdir=None, print_message=False, ckeckpoint=False):
        ...
        ## check output directory for file (if path is not NONE) OR write to sys.out (print)
    ...
    def __call__(self, simulation: 'Simulation'):
        if not self.running:
            ...
            base_message = (timestamp_str.ljust(13)
                            ...
                            + "  " + str(datetime_finish_estimate.strftime('%Y-%m-%d %H:%M:%S')).ljust(20))
            ...
            if self. checkpoint and self.t_elapsed > self.t_max:
                # checkpointin seems to be missing in current master...
                simulation.save_checkpoint(self.outdir + "/" + timestamp_str + "_f_" + str(simulation.flow.i) + ".cpt")

...
class NativeNoStreaming(NativeStreaming):
    ...
    def generate_read_write(self, generator: 'Generator'):
        if not generator.registered('read_write()'):
            ...
            generator.append_index_buffer(f'                                          ')
            generator.append_index_buffer(f'      index_t q_ = i;                     ')
            generator.append_index_buffer(f'      index_t x_ = index[0];              ', cond=d > 0)
            generator.append_index_buffer(f'      index_t y_ = index[1];              ', cond=d > 1)
            generator.append_index_buffer(f'      index_t z_ = index[2];              ', cond=d > 2)
            generator.append_index_buffer(f'                                          ')
            ...
            generator.append_write_buffer(f'                                          ')
            generator.append_write_buffer(f'      index_t q_ = i;                     ')
            generator.append_write_buffer(f'      index_t x_ = index[0];              ', cond=d > 0)
            generator.append_write_buffer(f'      index_t y_ = index[1];              ', cond=d > 1)
            generator.append_write_buffer(f'      index_t z_ = index[2];              ', cond=d > 2)
            generator.append_write_buffer(f'                                          ')
            generator.append_write_buffer(f'      f_next[{coord}] = f[i];             ')
            generator.append_write_buffer(f'    }}                                    ')
            generator.append_write_buffer(f'                                          ')
...
class NativeStandardStreaming(NativeStreaming):
    ...
    def generate_read_write(self, generator: 'Generator'):
        if not generator.registered('read_write()'):
            ...
            d = generator.stencil.stencil.D()
            coord = generator.lattice.get_lattice_coordinate(generator, ['q_', 'x_', 'y_', 'z_'][:d + 1])
            mask_coord = generator.lattice.get_lattice_coordinate(generator, ['x_', 'y_', 'z_'][:d])
            ...
            generator.append_write_buffer(f'                                                         ', cond=self.support_no_streaming_mask)
            generator.append_write_buffer(f'      index_t q_ = i;                                    ', cond=self.support_no_streaming_mask)
            generator.append_write_buffer(f'                                                         ', cond=self.support_no_streaming_mask)
            generator.append_write_buffer(f'      index_t x_ = index[0];                             ', cond=self.support_no_streaming_mask and d > 0)
            generator.append_write_buffer(f'      index_t y_ = index[1];                             ', cond=self.support_no_streaming_mask and d > 1)
            generator.append_write_buffer(f'      index_t z_ = index[2];                             ', cond=self.support_no_streaming_mask and d > 2)
            generator.append_write_buffer(f'                                                         ', cond=self.support_no_streaming_mask)
            generator.append_write_buffer(f'      f_next[{coord}] = f[i];                            ', cond=self.support_no_streaming_mask)
            generator.append_write_buffer(f'                                                         ', cond=self.support_no_streaming_mask)
            ...
            generator.append_write_buffer(f'            if (x_ <  0)            x_ += dimension[1];  ', cond=d > 1)
            generator.append_write_buffer(f'       else if (x_ >= dimension[1]) x_ -= dimension[1];  ', cond=d > 1)
            generator.append_write_buffer(f'                                                         ', cond=d > 1)
            generator.append_write_buffer(f'      index_t z_ = index[2] + e[i][2];                   ', cond=d > 2)
            generator.append_write_buffer(f'            if (x_ <  0)            x_ += dimension[2];  ', cond=d > 2)
            generator.append_write_buffer(f'       else if (x_ >= dimension[2]) x_ -= dimension[2];  ', cond=d > 2)

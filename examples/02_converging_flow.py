...
"""
For descriptions of the initialization, refer to '00_b_first_example_obstacle.py'.

Here, we use a lower Reynolds number for faster convergence. BGK also helps.
"""
...
while it <= nmax:
    ...
    print(f"avg MLUPS: {mlups / i:.3f}, avg energy: {energy_new:.8f}, rel. diff: {abs(energy_new - energy_old)/energy_old:.8f}")
    if not energy_new == energy_new:
        print("CRASHED!")
        break
    if abs(energy_new - energy_old)/energy_old < 0.0075:
        print(f"CONVERGED! To {abs(energy_new - energy_old)/energy_old/100:.2f}% after {it} iterations.")
        break

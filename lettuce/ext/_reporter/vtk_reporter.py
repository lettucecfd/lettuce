...
class VTKReporter(Reporter):
    ...
    def output_mask(self, no_collision_mask):
        ...
        point_dict = dict()
        if self.flow.stencil.d == 2:
            point_dict["mask"] = self.flow.context.convert_to_numpy(no_collision_mask)[..., None].astype(int)
        else:
            point_dict["mask"] = self.flow.context.convert_to_numpy(no_collision_mask).astype(int)

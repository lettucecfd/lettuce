...
class newsuperTGV3D:
    ...
    def __call__(self, f):
            ...
            for i in range(len(self.e)):
                for j in range(len(self.e)):
                    if self.e[i, 0] != 0 and self.e[i, 1] != 0 and self.e[i, 2] != 0 and self.e[i, 0] == -self.e[j, 0] and \
                            self.e[i, 1] == -self.e[j, 1] and self.e[
                        i, 2] == -self.e[j, 2]:
                        self.opposite.append((i, j))
            self.switch_stencil_corner.append(self.opposite)

        #####################################
        self.swap_w= np.array([[[0,slice(None),slice(None)],[-1,slice(None),slice(None)]]
        ...
        for i in range(6):
            for j in range(len(self.switch_stencil_array[i])):

                if i == 3:
                    f[self.switch_stencil_wall[i][j][1],self.swap_w[i,1,:]] = torch.transpose(self.f_copies[self.switch_stencil_wall[i][j][0],:, :, i],0,1)
                else:
                    f[self.switch_stencil_wall[i][j][1],]=self.f_copies[self.switch_stencil_wall[i][j][0],:,:,i]

        self.f_copies_borders=torch.stack((f[:,0,-1,:].clone(),f[:,-1,0,:].clone(),f[:,-1,-1,:].clone(),f[:,0,0,:].clone(),f[:,:,0,-1].clone()
                                           ,f[:,0,:,-1].clone(),f[:,:,-1,-1].clone(),f[:,-1,:,-1].clone(),f[:,:,-1,0].clone(),f[:,0,:,0].clone(),f[:,:,0,0].clone(),f[:,-1,:,0].clone()))

        self.borders=np.array([[0,-1,slice(None)],[-1,0,slice(None)],[0,0,slice(None)],[-1,-1,slice(None)],[slice(None),-1,0]
                                  ,[-1,slice(None),0],[0,slice(None),0],[slice(None),0,0],[slice(None),0,-1],[slice(None),-1,-1],[slice(None),0,0],[-1,slice(None),0]])
        for i in range(12):

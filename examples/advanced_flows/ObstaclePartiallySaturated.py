...
from lettuce.boundary import (EquilibriumBoundaryPU, EquilibriumOutletP,
...
lattice = lt.Lattice(lt.D3Q27 if dim == 3 else lt.D2Q9, device=default_device,
                     use_native=False)
Re = 250
Ma = 0.01
saturation = 0.5
...
class ObstaclePartially(lt.Obstacle):
    def __init__(self, shape, reynolds_number, mach_number, lattice,
        ...
        super().__init__(shape, reynolds_number, mach_number, lattice,
                         domain_length_x)
    ...
    def boundaries(self):
        ...
        return [
            ...
            EquilibriumOutletP(self.units.lattice,
                               self._unit_vector().tolist()),
            PartiallySaturatedBC(self.mask, self.units.lattice,
...
class Show2D:
    def __init__(self, lattice, mask, outdir: str, dpi: int = 1200,
        ...
        self.mask = self.lattice.convert_to_numpy(mask).transpose() \
            if (mask is torch.Tensor) else mask.transpose()
        self.dpi = dpi
        self.save = save
    ...
    def __call__(self, data, title: str, name: str, vlim=None):
        ...
        data = self.lattice.convert_to_numpy(data).transpose() \
...
simulation.reporters.append(lt.VTKReporter(lattice, flow, interval=200,
                                           filename_base=vtkdir))
simulation.initialize_f_neq()
show2d = Show2D(lattice, flow.mask, imgdir, save=False, show_mask=False,
                mask_alpha=saturation)
show2d(flow.units.convert_velocity_to_pu(lattice.u(simulation.f))[0],
       "u_x(t=0)", "u_0", vlim=(-.2, 1.5))
...
while steps < step_max and ~torch.isnan(simulation.f).any():
    ...
    rho = flow.units.convert_density_lu_to_pressure_pu(lattice.rho(
        simulation.f))  # [Pa]
    ...
    show2d(rho * 10 ** -5, f"density(it={steps},t={t:.1f}) [bar]",

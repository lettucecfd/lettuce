...
class DecayingTurbulence:
    ...
    def __generate_wavenumbers(self):
    ...
    def __generate_spectrum(self):
        wavenorms, wavenumber, wavemask = self.__generate_wavenumbers()
    ...
    def _generate_initial_velocity(self, ek, wavenumber):
        ...
        u = np.random.random(np.array(wavenumber).shape)*2*np.pi + 0j ## schönere Dimension?
        ...
        u_real_h = [np.sqrt(2 / self.units.lattice.D * ek / (u_imag[dim] ** 2 + u_real[dim] ** 2 + 1.e-15)) * u_real[dim] for dim in range(self.units.lattice.D)]
        u_imag_h = [np.sqrt(2 / self.units.lattice.D * ek / (u_imag[dim] ** 2 + u_real[dim] ** 2 + 1.e-15)) * u_imag[dim] for dim in range(self.units.lattice.D)]
        for dim in range(self.units.lattice.D):
        ...
        u_real = [u_real_h[dim] - divergence_real * wavenumber_modified[dim] / wavenorm_modified ** 2 for dim in range(self.units.lattice.D)]
        u_imag = [u_imag_h[dim] - divergence_imag * wavenumber_modified[dim] / wavenorm_modified ** 2 for dim in range(self.units.lattice.D)]
        for dim in range(self.units.lattice.D):
            u_real[dim].ravel()[0] = 0
            u_imag[dim].ravel()[0] = 0
        ...
        norm = self.resolution * dx ** (-2) * np.sqrt(self.units.characteristic_length_pu) if self.units.lattice.D == 3 else self.resolution / dx

        u = [(np.fft.ifftn(u_real[dim] + u_imag[dim] * 1.0j, axes=tuple((np.arange(self.units.lattice.D)))) * norm).real for dim in range(self.units.lattice.D)]
    ...
    def initial_solution(self, x):
        ...
        ek, wavenumber = self.__generate_spectrum()
